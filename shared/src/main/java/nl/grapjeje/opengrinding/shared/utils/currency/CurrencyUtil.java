package nl.grapjeje.opengrinding.shared.utils.currency;

import com.craftmend.storm.api.enums.Where;
import nl.grapjeje.core.text.MessageUtil;
import nl.grapjeje.opengrinding.shared.OpenGrinding;
import nl.grapjeje.opengrinding.shared.api.Currency;
import nl.grapjeje.opengrinding.shared.api.GrindingCurrency;
import nl.grapjeje.opengrinding.shared.core.CoreModule;
import nl.grapjeje.opengrinding.shared.core.objects.CraftGrindingCurrency;
import nl.grapjeje.opengrinding.shared.models.CurrencyModel;
...
public class CurrencyUtil {
    ...
    private static double getRemainingForToday(GrindingCurrency currency, Currency type) {
        if (type == Currency.TOKENS) return CoreModule.getConfig().getTokenLimit() - currency.getTokensFromToday();
    ...
    private static CompletableFuture<Map<Currency, Double>> giveCash(Player player, double amount, String reason) {
        return getModelAsync(player).thenApply(model -> {
            ...
            if (CoreModule.getConfig().isDailyLimit()) {
                allowed = getRemainingForToday(currency, Currency.CASH);
                if (allowed <= amount) {
                    Bukkit.getScheduler().runTask(OpenGrinding.getInstance(), () ->
    ...
    private static CompletableFuture<Map<Currency, Double>> giveTokens(Player player, double amount) {
        return getModelAsync(player).thenApply(model -> {
            ...
            double allowed;
            if (CoreModule.getConfig().isDailyLimit()) {
                allowed = getRemainingForToday(currency, Currency.TOKENS);
                if (allowed <= amount) {
                    Bukkit.getScheduler().runTask(OpenGrinding.getInstance(), () ->
